...
const bannerImg = useListBanner('clarionOverview')
...
/* 第一區（tablet）本身還留著整段 section 的上內距，84px 加上內距剛好卡在 subnav 下方；
   從第二區開始因為 .catsec + .catsec 把上內距拿掉了，內容整個貼齊區塊頂端，
   若還用同一個 84px，標題會被吸在頂端、卡進 sticky 導覽列底下造成疊字，
   所以要把 header(64px，語系橫條移除後的實際高度) + subnav 實際高度都算進去，抓大一點的距離。 */
...
.hero p { color: #41506b; max-width: 640px; font-weight: 300; }
