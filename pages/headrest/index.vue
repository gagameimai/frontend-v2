<template>
  <div class="sf-page">
    ...
    <div class="hero">
      ...
      <div class="wrap in">
        <div class="ey">{{ $t('headrest.eyebrow') }}</div>
        <h1>{{ $t('headrest.title') }}</h1>
        <p>{{ $t('headrest.intro') }}</p>
      </div>
    </div>
    ...
    <section>
      <div class="wrap">
        <div class="crumb">
          <NuxtLink to="/">{{ $t('headrest.home') }}</NuxtLink> ／ {{ $t('headrest.title') }}
        </div>

        <div class="shead">
          <div>
            <div class="sub">{{ $t('headrest.sectionKicker') }}</div>
            <div class="cn">{{ $t('headrest.sectionTitle') }}</div>
            <div class="desc">{{ $t('headrest.sectionDesc') }}</div>
          </div>
          <NuxtLink to="/partner" class="btn ghost">{{ $t('headrest.findInstaller') }}</NuxtLink>
        </div>
        ...
        <div class="grid">
          ...
          >
            <div class="img">
              <span class="badge">{{ $t('headrest.badge') }}</span>
            ...
            <div class="body">
              ...
              <div class="foot">
                <span class="ch">{{ $t('headrest.supplyByDealer') }}</span>
                <span class="ask">{{ $t('headrest.viewDetail') }}</span>
              </div>
            </div>
          </NuxtLink>
        </div>

        <p v-if="productList.length === 0" class="empty">{{ $t('headrest.empty') }}</p>
      </div>
    </section>
    ...
    <section class="help">
      <div class="wrap">
        <h2>{{ $t('headrest.helpTitle') }}</h2>
        <p class="help-p">{{ $t('headrest.helpDesc') }}</p>
        <div class="help-btns">
...
const bannerImg = useListBanner('headrest')

/**
 * 頭枕螢幕 API（後台尚未建立）
 * 待後端提供後把 ENDPOINT 改成實際路徑（例：'/headrest'）即可自動載入，版面不需調整。
 * 預期回傳格式與其他端點一致：{ result: [ { id, name, img, memo } ] }
 */
const ENDPOINT = '/headrest'

const { data } = await useAsyncData('headrest-list', () =>
  ENDPOINT
...
usePageSeo({
  title: () => seoT('headrest.title'),
  description: () => seoT('headrest.intro')
})
</script>
