<template>
  <div class="hrd-page">
    <!-- 麵包屑（白底，DVR/headrest 草稿樣式） -->
    <div class="wrap">
      <div class="crumb">
        <NuxtLink to="/">{{ $t('headrestDetail.home') }}</NuxtLink> ／
        <NuxtLink to="/headrest">{{ $t('headrestDetail.category') }}</NuxtLink> ／
        {{ info.name }}
      </div>
    </div>
    ...
    <div class="wrap">
      <div class="phero">
        ...
        <div class="pinfo">
          <div class="ey">{{ $t('headrestDetail.eyebrow') }}</div>
          <h1>{{ info.name }}</h1>
          <p class="sub">{{ info.memo_in }}</p>
          <div v-if="info.price" class="price-tag">
            <span class="pt-label">{{ $t('headrestDetail.specPrice') }}</span>
            <span class="pt-value">{{ info.price }}</span>
          </div>
          <div class="warranty">{{ $t('headrestDetail.warranty') }}</div>
          <div v-if="!info.price" class="price">
            {{ $t('headrestDetail.priceNote') }}　<span>{{ $t('headrestDetail.priceSub') }}</span>
          </div>
...
useHead({
  title: () => info.value.name || t('headrestDetail.title'),
  meta: [
...
.hrd-page a { color: inherit; text-decoration: none; }
.hrd-page img { display: block; max-width: 100%; }
