<template>
  <div class="au-page">
    ...
    <!-- 四大分類 -->
    <section v-for="(sec, si) in sections" :id="sec.key" :key="sec.key" class="catsec">
      <div class="wrap">
        <div class="shead">
          <div>
            <div class="sub">{{ $t(sec.kicker) }}</div>
            <div class="cn">{{ $t(sec.title) }}</div>
            <div class="desc">{{ $t(sec.desc) }}</div>
          </div>
          <NuxtLink v-if="si === 0" to="/partner" class="btn ghost">
            {{ $t('audio.findInstaller') }}
          </NuxtLink>
        </div>
        ...
        <div class="grid">
          <component
            ...
            v-bind="item.id ? { to: `/clarion/audioAccessoriesDetail/${item.id}` } : {}"
          >
            <div class="img">
              <span class="badge" :class="{ grey: item.soon }">
                {{ item.soon ? $t('audio.comingSoonBadge') : item.badge }}
              </span>
              <img
                v-if="item.img"
                :src="item.img"
            ...
            <div class="body">
              ...
              <div class="foot">
                <span class="ch" :class="{ dim: item.soon }">
                  {{ item.soon ? $t('audio.comingSoonNote') : $t('audio.supplyByDealer') }}
                </span>
                <span v-if="item.id" class="ask">{{ $t('audio.viewDetail') }}</span>
              </div>
            </div>
          </component>
        </div>

        <p v-if="sec.items.length === 0" class="empty">{{ $t('audio.empty') }}</p>

        <p v-if="si === sections.length - 1" class="note">{{ $t('audio.note') }}</p>
      </div>
    </section>
...
const eyebrow = computed(() => `${t('header.clarion')} ｜ ${t('audio.eyebrowSuffix')}`)

// Banner 背景圖（後台「列表頁 Banner 管理」；沒設定就用下面的預設漸層背景）
const bannerImg = useListBanner('audioAccessories')

/**
 * 汽車音響 API（後台尚未建立）
 * 待後端提供後，把 ENDPOINT 改成實際路徑（例：'/sound'）即可自動改用真實資料，
 * 版面不需調整。預期回傳格式與其他端點一致：{ result: [ { id, name, memo, img, group } ] }
 * 其中 group 對應下方 sections 的 key：speakers / tweeters / sub / amp
 */
const ENDPOINT = '/audio_accessories'

const { data } = await useAsyncData('clarion-audio', () =>
  ENDPOINT
...
const sections = computed(() =>
  SECTION_META.map((meta) => {
    const fromApi = apiList.value.filter((p) => Number(p.type) === meta.typeVal)
    return {
      ...meta,
      items: fromApi.map((p) => ({
        id: p.id,
...
const itemSpec = (item) => item.spec || ''

const onImgError = (e) => {
  e.target.style.display = 'none'
}

// sticky 側邊導覽：捲動到哪一區就把該區連結標成 active（比照 clarion/overview 的做法）
...
.au-page h2 { font-size: clamp(22px, 3vw, 30px); font-weight: 900; color: var(--ink); }
.wrap { max-width: 1080px; margin: 0 auto; padding: 0 26px; }
.au-page section { padding: 48px 0; }
.catsec + .catsec { padding-top: 0; }
/* 第一區（speakers）還留著 section 的上內距，118px 加上內距剛好卡在 subnav 下方；
   從第二區開始因為 .catsec + .catsec 把上內距拿掉了，內容整個貼齊區塊頂端，
   若還用同一個 118px，標題會被吸進 sticky 導覽列底下造成疊字，所以要抓大一點的距離。 */
#speakers { scroll-margin-top: 84px; }
#tweeters, #sub, #amp { scroll-margin-top: 138px; }
