<template>
  <div class="cam-page">
    ...
    <section>
      <div class="wrap">
        <div class="crumb">
          <NuxtLink to="/">{{ $t('camera.home') }}</NuxtLink> ／ {{ $t('camera.title') }}
        </div>

        <div class="shead">
          <div>
            <div class="sub">{{ $t('camera.sectionKicker') }}</div>
            <div class="cn">{{ $t('camera.sectionTitle') }}</div>
            <div class="desc">{{ $t('camera.sectionDesc') }}</div>
          </div>
          <NuxtLink to="/partner" class="btn ghost">{{ $t('camera.findInstaller') }}</NuxtLink>
        </div>
        ...
        <div class="grid">
          ...
          >
            ...
            <div class="body">
              ...
              <div class="foot">
                <span class="ch">{{ $t('camera.supplyByDealer') }}</span>
                <span class="ask">{{ $t('camera.viewDetail') }}</span>
              </div>
            </div>
          </NuxtLink>
        </div>
    ...
    <section class="help">
      <div class="wrap">
        <h2>{{ $t('camera.helpTitle') }}</h2>
        <p class="help-p">{{ $t('camera.helpDesc') }}</p>
        <div class="help-btns">
...
// 註：/camera API 後台尚未建立，待建好後即可自動載入，版面不需調整。
const BRANDS = {
  mm: { endpoint: '/camera', brand: 0, brandKey: 'header.mm', detailBase: '/mm/cameraDetail' },
  clarion: { endpoint: '/camera', brand: 1, brandKey: 'header.clarion', detailBase: '/clarion/cameraDetail' }
}
const cfg = computed(() => BRANDS[props.brand] || BRANDS.mm)

// Banner 背景圖（後台「列表頁 Banner 管理」，依品牌各自設定；沒設定就用下面的預設漸層背景）
const bannerImg = useListBanner('camera', computed(() => props.brand))

// 小標：分類名稱 ｜ 品牌（例：鏡頭 ｜ MM 美邁）
const eyebrow = computed(() => `${t('camera.title')} ｜ ${t(cfg.value.brandKey)}`)

// 取清單資料（Nuxt 3 SSR 寫法）：依網址品牌帶 brand 參數（mm=0 / clarion=1）
// ENDPOINT 尚未上線時回空陣列，版面照常顯示、清單先空。
const { data } = await useAsyncData(
  `camera-list-${props.brand}`,
  () =>
    cfg.value.endpoint
...
usePageSeo({
  title: () => `${t('camera.title')}｜${t(cfg.value.brandKey)}`,
  description: () => t('camera.intro'),
  brand: props.brand
})
</script>
...
.cam-page a { color: inherit; text-decoration: none; }
.cam-page img { display: block; max-width: 100%; }
.cam-page h2 { font-size: clamp(22px, 3vw, 30px); font-weight: 900; color: var(--ink); }
.cam-page .wrap { max-width: 1080px; margin: 0 auto; padding: 0 26px; }
.cam-page section { padding: 48px 0; }
