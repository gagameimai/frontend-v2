<template>
  <div class="camd-page">
    ...
    <div class="wrap">
      <div class="crumb">
        <NuxtLink to="/">{{ $t('cameraDetail.home') }}</NuxtLink> ／
        <NuxtLink :to="cfg.listRoute">{{ $t('cameraDetail.category') }}</NuxtLink> ／
        {{ info.name }}
      </div>
    </div>
      ...
      <div class="phero">
        ...
        <div class="pinfo">
          ...
          <div v-if="info.price" class="price-tag">
            <span class="pt-label">{{ $t('cameraDetail.specPrice') }}</span>
            <span class="pt-value">{{ info.price }}</span>
          </div>
          <div class="warranty">{{ $t('cameraDetail.warranty') }}</div>
          <div v-if="!info.price" class="price">
            {{ $t('cameraDetail.priceNote') }}　<span>{{ $t('cameraDetail.priceSub') }}</span>
          </div>
...
const BRANDS = {
  mm: { endpoint: '/camera', brand: 0, brandKey: 'header.mm', listRoute: '/mm/camera' },
  clarion: { endpoint: '/camera', brand: 1, brandKey: 'header.clarion', listRoute: '/clarion/camera' }
}
const cfg = computed(() => BRANDS[props.brand] || BRANDS.mm)
const eyebrow = computed(() => `${t('cameraDetail.category')} ｜ ${t(cfg.value.brandKey)}`)
...
useHead({
  title: () => info.value.name || t('cameraDetail.title'),
  meta: [
...
.camd-page a { color: inherit; text-decoration: none; }
.camd-page img { display: block; max-width: 100%; }
